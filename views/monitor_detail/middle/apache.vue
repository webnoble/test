              ...
              <div class="round_box">
                ...
                <div class="info_box">
                  <div class="prompt one">
                    当前每秒请求数:
                    <span class="size">{{ $common.getNumber(detailData.reqPerSec) || 0 }}个</span>
                  </div>
                  <div class="prompt two">
                    运行线程数:
                    <span class="size">{{ detailData.busyWorkers || '0' }}个</span>
                  </div>
                  <div class="prompt three">
                    空闲线程数:
                    <span class="size">{{ detailData.idleWorkers || '0' }}个</span>
                  </div>
                  <div class="prompt four">
                    总字节数:
                    <span class="size">{{ detailData.totalKbytes || '0' }}个</span>
                  </div>
                  <div class="prompt five">
                    数据发送速率:
                    <span class="size">{{ detailData.sendRate || '0' }}kb/s</span>
                  </div>
                  <div class="prompt six">
                    数据接收速率:
                    <span class="size">{{ detailData.receiveRate || '0' }}kb/s</span>
                  </div>
                </div>
              </div>
            </el-col>
...
<script>
// import pie from './pie';
import { getApacheDetail } from '@/api/middleDetail';
export default {
  name: 'Apache',
  // components: {
  //   pie
  ...
  methods: {
    getData () {
      getApacheDetail(this.$route.query.cmId).then((res) => {
        this.detailData = res;
        this.chartsData.x = this.formatCharts(
          this.detailData.reqPerSecList
        ).x;
