...
export default {
  ...
  methods: {
    ...
    start (data) {
      this.$request.post(`/ruleMailInfo/updateMailRuleStatusById?id=${data.id}&status=${1}`).then(() => {
        this.$refs.table.getList();
      })
    },
    stop (data) {
      this.$request.post(`/ruleMailInfo/updateMailRuleStatusById?id=${data.id}&status=${0}`).then(() => {
        this.$refs.table.getList();
      })
    },
    deleteItem (data) {
