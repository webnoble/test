...
export default {
  ...
  getEvents () {
    return {
      ...
      'mouseup': 'onMouseup',
      'combo:drop': 'onDrop',
      'combo:dragenter': 'onComboDragenter',
      'combo:dragleave': 'onComboDragleave',
      'combo:dragend': 'onComboDragend'
    };
  },
  onComboDragend (e) {
    console.log('drag-item->onComboDragend:' + e.name);
    this.graph.getCombos().forEach(combo => {
      this.graph.setItemState(combo, 'dragenter', false);
    })
  },
  onComboDragenter (e) {
    console.log('drag-item->onComboDragenter:' + e.name);
    this.graph.setItemState(e.item, 'dragenter', true);
  },
  onComboDragleave (e) {
    console.log('drag-item->onComboDragleave:' + e.name);
    this.graph.setItemState(e.item, 'dragenter', false);
  },
  onDrop (e) {
    console.log('drag-item->onDrop:' + e.name);
    this.graph.setItemState(e.item, 'dragenter', false);
    // 响应拖动node到comb里面
    const combo = e.item;
    const node = this.nodeEvent.item;
    combo.addChild(node);
    this.graph.paint();
  },
  onMouseup (e) {
  ...
  onDragend (e) {
    ...
    this.target = null;
    // 终止时需要判断此时是否在监听画布外的 dragend 事件，若有则解绑
    const fn = this.fn;
    if (fn) {
      body.removeEventListener('dragend', fn, false);
      this.fn = null;
    }
    this.isDrag = false
    this.nodeEvent = null
