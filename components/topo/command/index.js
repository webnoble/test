...
class Command {
  ...
  unGroup (combo) {
    const graph = this.editor.getGrpah();
    combo._cfg.model.children.forEach((item) => {
      const node = graph.findById(item.id);
      const model = JSON.parse(JSON.stringify(node.getModel()));
      model.comboId = '';
      this.executeCommand('add', [model]);
    });
    this.executeCommand('delete', [combo]);
  }
  undo () {
